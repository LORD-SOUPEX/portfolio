...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const init3DLogo = () => {
        ...
        // Letter A (Simplified Bars)
        const leftBarGeom = new THREE.BoxGeometry(0.3, 2.5, 0.3);
        const leftBar = new THREE.Mesh(leftBarGeom, material);
        leftBar.position.x = -0.6;
        leftBar.rotation.z = 0.3;
        group.add(leftBar);

        const rightBarGeom = new THREE.BoxGeometry(0.3, 2.5, 0.3);
        const rightBar = new THREE.Mesh(rightBarGeom, material);
        rightBar.position.x = 0.6;
        rightBar.rotation.z = -0.3;
        group.add(rightBar);

        const crossBarGeom = new THREE.BoxGeometry(1, 0.2, 0.3);
    ...
    const initPlaybookEditor = () => {
        ...
        const typePlaybook = () => {
            ...
            div.style.color = line.c === '#keyword' ? '#ff7b72' : (line.c === '#prop' ? '#79c0ff' : '#a5d6ff');
            if (line.c === '#comment') div.style.color = '#8b949e';
        ...
        const runAnsible = () => {
            const logs = [
                ...
                { t: 'ok: [web1.brice.fr]', c: 'cyan' },
                { t: 'TASK [Ensure Nginx is installed] *********************************', c: '#fff' },
                { t: 'changed: [web1.brice.fr]', c: 'yellow' },
                { t: 'TASK [Start Nginx service] ***************************************', c: '#fff' },
                { t: 'ok: [web1.brice.fr]', c: 'cyan' },
                { t: 'PLAY RECAP *******************************************************', c: '#fff' },
                { t: 'web1.brice.fr : ok=3 changed=1 unreachable=0 failed=0', c: 'green' }
            ];
